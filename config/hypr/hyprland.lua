...
hl.bind(mainMod .. "+" .. "Q", hl.dsp.window.close(), { description = "Закрыть окно тоже" })
...
hl.bind("SHIFT + Print", hl.dsp.exec_cmd("grim ~/Screenshot_$(date +%Y-%m-%d_%H-%M-%S).png"), { description = "Скриншот всего экрана" }) 
--можно изменить путь сохранения скриншотов!    ↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑
...
-- Яркость экрана (нужен пакет brightnessctl + пользователь в группе video)
...
-- Mute микрофона (отдельная клавиша, не путать с XF86AudioMute -> это звук динамиков)
hl.bind("XF86AudioMicMute", hl.dsp.exec_cmd("wpctl set-mute @DEFAULT_AUDIO_SOURCE@ toggle"), { locked = true })
 
-- Кнопка Lock на клавиатуре. У большинства ноутбуков (в т.ч. Lenovo) физическая
-- кнопка с иконкой замка шлёт именно XF86ScreenSaver. Если не сработает —
-- определите реальное имя клавиши через `wev` (нажать клавишу, посмотреть в выводе
-- поле keysym) и замените строку ниже на то, что реально прилетает.
...
hl.on("hyprland.start", function()
    ...
    hl.exec_cmd("waybar")
    hl.exec_cmd("~/.config/hypr/scripts/get_hyprpaper.sh")
    hl.exec_cmd("mako")
