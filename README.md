...
|**brightnessctl**| Контроль яркости на ноутбуках |
|**mako**| Системные уведомления |

Основной браузер - Firefox, есть примеры приложений из автозапуска.
...
- **Простой/блокировка/сон** — трёхступенчато через Hypridle: выключение экрана → блокировка → сон, но оба первых шага **ждут окончания медиа** (проверка через `playerctl`), чтобы не гасить экран во время просмотра видео или прослушивания музыки.
- **Шрифт** — везде (панель, лаунчер, экран блокировки, терминал) единый JetBrains Mono с полной поддержкой кириллицы.
- **Обои** — обои случайным образом выбираются из папки `~/.config/hypr/wallpapers` скриптом при запуске сессии, в папку можно поместить изображения для добавления в ротацию.
